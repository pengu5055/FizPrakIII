...
x = np.genfromtxt("SDS00004.txt", usecols=0)
y = np.genfromtxt("SDS00004.txt", usecols=1)

fig, ax = plt.subplots()
fitpar, fitcov = curve_fit(theoreticalfit, xdata=x, ydata=y, p0=[0, 0, -0, 7.6])
...
plt.title("Razbremenitev z maso 502g")
plt.xlabel("t [s]")
plt.ylabel("U [V]")
plt.text(0.49, 0.275, fittext, ha="left", va="center", size=10, transform=ax.transAxes, bbox=dict(facecolor="#a9f5ee", alpha=0.5))
plt.legend(loc=0)
plt.show()
